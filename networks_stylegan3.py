...
class SynthesisInput(torch.nn.Module):
    ...
    def forward(self, w, style=None):
        ...
        if style is None:
            style = self.affine(w) # t = (r_c, r_s, t_x, t_y)
        t = style
...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='const', force_fp32=False, update_emas=False, style=None):
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, s=None, return_maps=False, return_s=False, **layer_kwargs):
        if ws is None and s is not None:
            ws = torch.zeros([s.shape[0], self.num_ws, self.w_dim]).cuda()

        misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
        ws = ws.to(torch.float32).unbind(dim=1)
        # Execute layers
        if s == None:
            x, style = self.input(ws[0])
        else:
           x, style = self.input(ws[0], style=s[:, :4]) 
        ...
        for name, w in zip(self.layer_names, ws[1:]):
            ...
            if s is not None:
                x, style = layer(x, w, style=s[:, n:n+layer.in_channels])
                n += layer.in_channels
            else:
                x, style = layer(x, w, **layer_kwargs)
            maps.append(x)
            styles.append(style)
        ...
        if return_maps and return_s:
            return x, maps, styles
        elif return_s:
            return x, styles
        elif return_maps:
            return x, maps
